...
function stringToCode128(text) { 
  ...
  barc.currcs = perhapsCodeC(bytes, barc.currcs);  //if no codeset changes this will end up with bytes.length+3  //start, checksum and stop  
  var codes = new Array(); 
  switch (barc.currcs) {  
    case CODESET.A:
  ...
  for (var i = 0; i < bytes.length; i++) {  
    var b1 = bytes[i]; //get the first of a pair    //should we translate/replace    
  ...
  return codes; 
  function getBestStartSet(csa1, csa2) {   //tries to figure out the best codeset
  ...
  function codesForChar(chr1, chr2, currcs) {  
    ...
    if (charCompatible(chr1, currcs)) {   
      if (currcs == CODESET.C) {    
        ...
        else if ((chr2 != -1) && !charCompatible(chr2, currcs)) {      //need to check ahead as well  
              
    ...
    else {    //if there is a next char AND that next char is also not compatible    
       
      if ((chr2 != -1) && !charCompatible(chr2, currcs)) {     //need to switch code sets 
           
        switch (currcs) {     
          case CODESET.A:
                  shifter = SET_CODEB;      
      ...
      else {     //no need to shift code sets, a temporary SHIFT will suffice        
        shifter = SET_SHIFT;   
      }  
    ...
    if (shifter != -1) {   
      ...
      if (currcs == CODESET.C) {     //include next as well  
           
